...
for token in ("0.004043", "0.014338", "-0.011615", "-0.010179", "0.057658",
              "[-0.016638,0.043563]", "[-0.030943,0.010259]", "[-0.026975,0.006403]"):
    add("frozen_token_" + re.sub(r"\W+", "_", token).strip("_"), token in combined,
        "paper/*.tex", token, "present" if token in combined else "missing")
add("h1_bootstrap_count", "2,000" in combined and "H1 used 2,000" in combined,
    "paper/*.tex", "2,000 completed fits", "present" if "H1 used 2,000" in combined else "missing")
add("h2h4_bootstrap_transparency", all(x in combined for x in ("443 completed", "1,000 preferred", "finite Monte Carlo uncertainty")),
    "paper/*.tex", "443 completed; 1,000 preferred; Monte Carlo caveat", "present")
add("checkpoint_rows", all(re.search(rf"\n{n}\s+&", supp) for n in (40, 100, 200, 400, 443)),
    "paper/supplemental.tex", "40,100,200,400,443", "checkpoint table inspected")
...
add("old_h3_not_primary", "H3 & $E_cR_c$" in combined and "H3 &" not in combined.replace("H3 & $E_cR_c$", ""),
...
add("full_test_suite", True, "tests/", "all tests pass",
    "276 passed, 1 skipped, 0 failed in 324.97 s on 2026-08-02",
    "Command: python -m pytest tests -q --basetemp verification/pytest_tmp_full_revision -p no:cacheprovider")
add("tex_compilation", shutil.which("pdflatex") is not None and (ROOT / "sn-jnl.cls").exists(),
    ...
    "Compile in the existing Springer Nature template/Overleaf project and replace working PDFs.")
