...
\abstract{\textbf{Purpose:} Trainability interventions for quantum neural networks are usually evaluated one at a time, even though architecture, objective, and connectivity choices can jointly determine whether a finite-shot gradient is usable. This study tests whether restricted entanglement, a local transverse-field-Ising-model energy objective, and a residual shortcut reinforce, duplicate, or interfere.

\textbf{Methods:} All eight configurations of the three binary strategies are evaluated in a four-qubit measure--re-encode--reset hybrid quantum neural network across matched block-count and shot-budget sweeps. Outcomes include exact-gradient magnitude, end-to-end estimator signal-to-noise ratio (SNR), bias, sign agreement, task metrics, and measurement cost. Confirmatory mixed-effects models use Holm correction, supplemented by bootstraps and sensitivity analyses.

\textbf{Results:} Restricted entanglement and the local objective show a positive exact-gradient interaction ($\eta_{EL} = 0.0043$, $p_{\text{Holm}} = 0.013$), corroborated by a matched bootstrap. The analogous end-to-end estimator-SNR interaction is positive and rejected under prespecified Wald/Holm inference ($\beta_{EL} = 0.025$, $p_{\text{Holm}} = 0.002$), but this significance is not corroborated by three independent robustness checks, and its sign varies by block count. An independent replication reproduces the positive pooled direction at roughly twice the magnitude, with partial depth-specific agreement. No $E \times R$ interaction is detected; the $L \times R \times$ block-count interaction is not rejected but is mode- and depth-sensitive. Combining all three strategies does not consistently outperform the best single strategy.

\textbf{Conclusion:} Interaction-aware evaluation reveals composition effects that isolated ablations can miss. In this four-qubit noiseless case study, the positive exact-gradient $E \times L$ interaction remains the most robust finding, while the analogous finite-shot evidence is inference-method dependent and heterogeneous across block count.}
...
\subsection{Post-run H2 robustness, replication, and depth-heterogeneity analyses}
\label{sec:h2-depth-methods}

Following the confirmatory H2--H4 fit, a package of post-run analyses examined the $\beta_{EL}$ estimate from several directions not specified in advance. None of these carries Holm correction, none joins the original H1--H4 family, and none changes the prespecified rejection decision reported in Section~\ref{sec:results-robustness}; each is a robustness, replication, or sensitivity check on an already-decided confirmatory result. Full methods, code, and machine-readable outputs are in \ttf{verification/h2\_robustness\_replication\_plan.md} and \ttf{verification/h2\_depth\_heterogeneity\_plan.md} (both written and frozen before the corresponding results were inspected).

\textbf{Numerator/denominator decomposition.} Because $\mathrm{SNR}_{\mathrm{est}}$ is a ratio, its $E{:}L$ interaction was decomposed into a numerator model ($\operatorname{asinh}|\bar g|$, the repeated-shot gradient-mean magnitude) and a denominator model ($\log$ repeated-shot SD), fit with the identical fixed-effect structure on the identical eligible rows.

\textbf{Cluster-robust reanalysis.} The identical fixed-effect specification was refit as ordinary least squares with standard errors clustered by initialization (50 clusters), a heteroscedasticity- and within-cluster-correlation-robust alternative that does not assume the mixed model's parametric variance structure.

\textbf{Independent initialization-level resampling.} A separately implemented resampling procedure (not the production bootstrap code) redrew complete initializations with replacement, resampled replicates within each cell, recomputed pointwise statistics, and refit the model, with the number of cells landing on exactly zero variance logged explicitly at every iteration ($n=50$ iterations; iteration count fixed for computational feasibility, not for a preferred outcome, before any coefficient was inspected).

\textbf{Independent replication.} A second, independently seeded dataset (seed root distinct from and verified non-overlapping with the original) was generated under the identical factorial design, block-count and budget sweep, and initialization count ($R_{\mathrm{rep}}=30$, matching the original exactly), then analyzed with the identical pipeline.

\textbf{Continuous depth-moderation model.} The adopted H2--H4 formula was extended with one additional term, $E{:}L{:}\widetilde D$, to test whether the $E{:}L$ interaction varies linearly with centered block count.

\textbf{Categorical depth-moderation model.} Block count was alternatively coded categorically (\ttf{C(depth, Sum)}, deviation coding over $D\in\{1,2,3,4,6\}$) with an $E{:}L{:}C(D)$ moderation term added to the same adopted structure (no $E{:}R{:}C(D)$ or $E{:}L{:}R{:}C(D)$ term was added, so any depth-dependence this model can express in the $E{:}L$ contrast is, by construction, not itself allowed to vary with $R$). Depth-specific $E{:}L$ contrasts were built as explicit linear combinations of the fitted design matrix (never by reading off a coefficient under an assumed reference-category meaning), marginalized equally over $R$, with standard errors and confidence intervals taken from the model's full fixed-effect covariance matrix. A joint Wald test on all four $E{:}L{:}C(D)$ coefficients (programmatically identified from the fitted model's own design information, never by a hardcoded coefficient position) tests whether a single model supports depth-varying $E{:}L$ at all.

\textbf{Equal-depth and observation-count weighted contrasts.} The five depth-specific contrasts were pooled two ways: equally ($1/5$ per depth) and weighted by each depth's eligible observation count. The two weightings are reported side by side with the adopted, fully-pooled coefficient; observation-count weighting gives more influence to deeper block counts because they contribute more matched quantum parameters per initialization, not because they are more reliable, and neither weighting is expected to equal the adopted model's own (REML-implied) weighting exactly.

\textbf{Include-zero-variance sensitivity.} For the original dataset only, the 509 zero-variance cells (each independently verified to have a repeated-shot mean of exactly zero, not merely a tied nonzero value) were included as $\mathrm{SNR}_{\mathrm{est}}=0$ rather than excluded, with no epsilon or variance floor, and every model above was refit on this alternative eligible set as a labeled sensitivity analysis.
...
The adopted end-to-end nested bootstrap was extended from an initial $n=40$ to a final $n=443$ completed iterations (0 failed fits), exceeding the required minimum of 400 and falling short of the preferred target of 1000 achieved under a documented resource ceiling (bootstrap scaling and shard provenance are detailed in the reproducibility index, Table~\ref{tab:repro-index}). The final percentile interval is $[-0.018024,0.065688]$ (median $0.023685$), which includes zero. The prespecified Wald/Holm rejection remains the confirmatory decision. The bootstrap does not independently corroborate H2, and this is no longer attributed to low iteration count or coarse endpoint resolution: across the roughly tenfold increase from $n=40$ to $n=443$, the interval did not narrow or move away from zero, and its width increased slightly (from $0.068$ to $0.084$). H2 is therefore supported by the prespecified Wald/Holm procedure but is not corroborated by the nested percentile bootstrap.

\textbf{Post-run robustness (Section~\ref{sec:h2-depth-methods}).} Decomposing $\mathrm{SNR}_{\mathrm{est}}$ shows the component decomposition is stable and directionally coherent: the gradient-mean magnitude increases under $E{=}L{=}1$ ($\operatorname{asinh}|\bar g|$ coefficient $+0.004315$, 95\% CI $[0.003076,0.005554]$) and the repeated-shot SD decreases ($\log$-SD coefficient $-0.149866$, 95\% CI $[-0.173538,-0.126194]$); both are individually distinguishable from zero and both act in the direction that increases $\mathrm{SNR}_{\mathrm{est}}$, with no sign reversal under any of 50 leave-one-initialization-out deletions. Three sensitivity analyses---the existing nested bootstrap above, a separately implemented initialization-level resampling procedure (percentile interval $[-0.003038,0.066606]$, $n=50$), and an alternative cluster-robust regression (95\% CI $[-0.021739,0.068839]$)---produce intervals including zero; each relaxes a different, specific assumption of the mixed model (resampling variability, an independent reimplementation, and the homoscedasticity assumption respectively), and none of the three is a universal check of every mixed-model assumption at once.

A continuous depth-moderation term is distinguishable from zero in the original data ($E{:}L{:}\widetilde D=+0.024952$, 95\% CI $[0.014438,0.035466]$), and a categorical depth model's joint Wald test on all four $E{:}L{:}C(D)$ coefficients rejects the null of no depth-varying interaction overwhelmingly ($\chi^2=154.05$, $\mathrm{df}=4$, $p=2.8\times10^{-32}$)---evidence that a single model supports treating $E{:}L$ as depth-dependent rather than uniform. Every one of the five depth-specific contrasts individually excludes zero, with a sign reversal at shallow block count: $-0.082$ ($D=1$) and $-0.118$ ($D=2$), versus $+0.093$, $+0.037$, and $+0.019$ at $D=3$, $4$, and $6$. This joint-model omnibus test does not survive the cluster-robust reanalysis ($\chi^2=6.71$, $p=0.15$), so, as with the pooled coefficient itself, the evidence for depth-varying $E{:}L$ is real under the mixed model's assumptions and does not survive relaxing them; the $D=2$ reversal's sign is preserved under cluster-robust inference (point estimate $-0.121$) without remaining individually significant, and is essentially unchanged under the include-zero-variance sensitivity treatment (still significant at every depth). The independent replication's own joint test is also significant ($\chi^2=37.77$, $p=1.3\times10^{-7}$), but every one of its five depths is positive---the shallow-depth sign reversal does not appear in the replication, so the two datasets' depth-specific patterns agree at 3 of 5 depths ($D=3,4,6$) and disagree at 2 ($D=1,2$), the same two depths where this study's replicate-count calibration was already weakest (Section~\ref{sec:results-calibration}). Equal-depth and observation-count-weighted pooled contrasts disagree materially in the original data (95\% CI $[-0.022,0.002]$ versus $[0.004,0.024]$), underscoring that the adopted, fully-pooled coefficient reflects neither weighting scheme uniquely. Figure~\ref{fig:h2-depth-heterogeneity} and Table~\ref{tab:h2-depth-heterogeneity} report the complete set of contrasts, omnibus tests, and weighted pools for both datasets.
...
\textbf{Post-run depth-heterogeneity analysis.} Figure~\ref{fig:h2-depth-heterogeneity} and Table~\ref{tab:h2-depth-heterogeneity} report the full post-run depth-moderation package summarized in the H2 results above (Section~\ref{sec:h2-depth-methods}): the continuous and categorical moderation models, the joint omnibus tests (mixed-model and cluster-robust) for whether $E{:}L$ varies with block count, the five depth-specific contrasts for both the original and independently replicated datasets, and the equal-depth versus observation-count weighted pooled contrasts. None of this is multiplicity-adjusted or part of the confirmatory family in Table~\ref{tab:confirmatory-summary}, which is unchanged; observation-count weighting is reported alongside equal-depth weighting specifically because deeper block counts contribute more matched quantum parameters per initialization and therefore more rows to that weighting, not because they are more trustworthy.

\begin{figure*}[t]
\centering
\includegraphics[width=0.92\textwidth]{figures/fig11_h2_depth_heterogeneity.pdf}
\caption{Post-run, exploratory depth-heterogeneity analysis of the $E{:}L$ interaction (not multiplicity-adjusted; does not alter the confirmatory decision in Table~\ref{tab:confirmatory-summary}). (a) Depth-specific $E{:}L$ contrasts (categorical mixed model, 95\% Wald CIs) for the original data and an independently seeded replication, fit separately and never pooled. The original shows a sign reversal at $D=1,2$; the replication is positive at every depth. (b) Equal-depth-weighted, observation-count-weighted, adopted (fully-pooled), and replication pooled-Wald estimates, both datasets. Equal-depth and observation-weighted contrasts disagree materially in the original data.}
\label{fig:h2-depth-heterogeneity}
\end{figure*}

\input{tables/h2_depth_heterogeneity.tex}

\section{Discussion}

\subsection{Summary and effect-size interpretation}

Two of the four confirmatory hypotheses are rejected under the prespecified Wald/Holm procedure and two are not. H1 and H2 have the same positive direction, providing model-based evidence of a super-additive $E\times L$ interaction in both exact-gradient magnitude and end-to-end estimator SNR. The achieved $n=443$ end-to-end bootstrap did not independently corroborate H2, stably rather than as an artifact of too few iterations, and this pattern is corroborated further by a cluster-robust reanalysis and an independently implemented resampling procedure (Section~\ref{sec:h2-depth-methods}), both of which likewise produce intervals including zero. A post-run depth-heterogeneity analysis additionally shows the interaction's sign is not uniform across block count in the original data, though an independent replication reproduces its positive pooled direction. The finite-shot result should therefore be read as Wald/Holm-rejected, mechanistically coherent (Section~\ref{sec:h2-depth-methods}), and reproducible in pooled direction under new seeds, but not corroborated by relaxing the mixed model's variance assumptions and not uniform across the depths it pools over---distinct claims that should not be collapsed into either ``confirmed'' or ``not robust.'' On the secondary multiplicative indices, the interaction corresponds to a 24.2\% shift in RMS exact-gradient magnitude ($J_{EL}=1.242$) and a 3.4\% shift in RMS $\snrest$ ($I_{EL}=1.034$): directionally consistent, but operationally modest on the SNR scale (Figure~\ref{fig:el-primary}). H3 yields an estimate close to zero and no evidence of an $E\times R$ interaction, although equivalence was not tested. H4 is not rejected, and its proximity to the unadjusted threshold is fragile to the analyzed depth range, estimator mode, and leave-one-initialization-out perturbation; the $n=443$ bootstrap remains genuinely inconclusive rather than a result awaiting more resampling.

\subsection{Evaluation of the mechanistic prior}

The companion review's directional expectation was that restricted entanglement and the local TFIM-energy objective would interact \emph{sub-additively} at the exact-gradient level: an architecture that already preserves locally relevant derivative structure might leave the local objective less marginal signal to add \cite{shen2026gradient}. The data contradict that expectation. $\eta_{EL}$ is consistently positive, and the pooled finite-shot estimator-SNR coefficient has the same direction, though the finite-shot version is not uniform: a post-run depth-heterogeneity analysis (Section~\ref{sec:h2-depth-methods}) finds the sign reverses at the two shallowest block counts and is positive from $D=3$ onward, so the pooled $\beta_{EL}$ should not be read as implying one universal $E\times L$ sign across depth for the finite-shot estimator, even though the exact-gradient $\eta_{EL}$ shows no comparable evidence of depth-dependence. The result is therefore not evidence that the two interventions merely duplicate a shared mechanism. Within this architecture, restricting within-block correlation spreading and changing which reduced-state derivatives enter the objective appear to reinforce one another.
...
Finally, the zero-variance exclusion pattern exposes a limitation specific to plug-in SNR estimation. All excluded cells occur under the global-cost condition, making the finite-shot $E\times L$ model conditional on factor-dependent eligibility. Including those cells as zero-SNR rather than excluding them shifts $\beta_{EL}$ without reversing it or changing which depths are individually significant, which is reassuring on that specific point but does not identify the exclusion mechanism, eliminate selection concerns more broadly, or bear on the depth-heterogeneity finding above. This caveat is one reason the exact-gradient interaction, which does not depend on that exclusion process, shows no comparable depth-dependence, and is corroborated by the matched bootstrap, remains the paper's strongest result.
...
The depth-heterogeneity analysis (Section~\ref{sec:h2-depth-methods}) illustrates a related, second-order version of the same point: a single pooled interaction coefficient can itself conceal effect modification across a third factorial dimension. Here, one pooled $\beta_{EL}$ summarizes five block counts whose individual $E\times L$ contrasts disagree in sign, and a single joint model is needed to detect this rather than assuming the pooled estimate applies uniformly. Factorial evaluations of QNN interventions should therefore budget for testing whether an interaction moderates, or is itself moderated by, architecture-size variables such as block count, not only whether the interaction exists on average.
...
The planned $D=1$-exclusion and exclusion-rate sensitivity checks (Section~\ref{sec:zero-variance-sensitivity}) have since been completed, and the zero-variance selection concern they were meant to address turned out to be more serious than initially anticipated: exclusion is not merely concentrated at $D=1$, it is exactly confined to $L=0$ across the entire design, in both estimator modes. $L$ is one of the two factors in the paper's headline $E\times L$ interaction, so the confirmatory estimator-SNR model is conditional on an eligibility process perfectly associated with cost type. This does not establish that $\beta_{EL}$ is biased by it---the $D=1$-exclusion refit moves $\beta_{EL}$ \emph{away} from, not toward, zero, the opposite of what a spurious, exclusion-driven signal would be expected to do---but the completed analysis does not rule out a subtler mechanism either, and the selection process's physical or computational origin remains uninvestigated. $\beta_{EL}$ also shifts by approximately $1.0$--$1.1$ original standard errors under two independent perturbations (excluding $D=1$; deleting the single most influential initialization), a magnitude worth carrying forward as an open precision/influence caveat even though neither perturbation reverses its sign or its rejection status. The adopted end-to-end H2--H4 bootstrap was extended to a final $n=443$ (Section~\ref{sec:results-robustness}), exceeding the required minimum of 400 but short of the preferred target of 1000; at this achieved scale, the bootstrap does not independently corroborate H2 and remains inconclusive for H4, a pattern that held stably across the extension rather than resolving with more iterations. Residual diagnostics on the adopted fit also show real heteroscedasticity by block count and budget and moderately heavy tails (Section~\ref{sec:results-robustness}), meaning the model-based Wald standard errors rest on an imperfect homogeneous-residual approximation; a cluster-robust reanalysis was subsequently added as a post-run sensitivity check (Section~\ref{sec:h2-depth-methods}), but no variance-function or heteroscedasticity-weighted refinement of the mixed model itself was undertaken. Finally, every result here is from noiseless simulation; no hardware-noise experiment was performed, and no training-trajectory (as opposed to matched-initial-parameter) outcome was evaluated.

The post-run H2 robustness and depth-heterogeneity package (Section~\ref{sec:h2-depth-methods}) has its own limitations, reported here rather than left implicit. All 509 original zero-variance end-to-end cells were independently verified to have a repeated-shot mean of exactly zero, not merely a tied nonzero value; including them as zero-SNR rather than excluding them moves $\beta_{EL}$ by roughly one original standard error without reversing its sign, so this specific sensitivity check does not resolve the broader selection question raised above. Three independently derived robustness intervals---the existing nested bootstrap, a cluster-robust reanalysis, and a separately implemented initialization-level resampling procedure---include zero, and a categorical depth model shows the $E\times L$ interaction's sign is not uniform across block count in the original data, with a joint omnibus test that itself does not survive cluster-robust reanalysis. An independently seeded replication reproduces the pooled interaction's positive sign at roughly twice the original magnitude and agrees with the original's depth-specific pattern at 3 of 5 block counts, disagreeing at the two shallowest, where this study's own replicate-count calibration was already documented as weakest; whether this reflects calibration noise in the original data, in the replication, or a genuine but unstable finite-shot phenomenon is not resolved here. Both datasets have only 50 independent initialization clusters, limiting the precision of any cluster-robust or resampling-based interval regardless of total row count. The replication's own bootstrap ($n=30$) is a preliminary check calibrated for computational feasibility and should not be treated as comparable in precision to the original's $n=443$ bootstrap. No hierarchical or zero-inflated model for the zero-variance-cell mechanism was implemented, for lack of a vetted dependency in the existing statistical stack, not because one was attempted and found unnecessary.
...
Several directions follow directly from this study's scope limits and from the depth-heterogeneity findings above (Section~\ref{sec:h2-depth-methods}). The most direct priority is more independent initializations: both the original and replication datasets have only 50 initialization clusters, which limits cluster-robust and resampling-based precision regardless of how many replicates or shots are collected per cell, so additional independent initializations---not simply a larger $R_{\mathrm{rep}}$---are the more direct route to resolving whether the pooled $E\times L$ estimate is well-determined. Closely related, an explicit, prespecified replication of the depth-moderation pattern itself (rather than the pooled coefficient alone) would test directly whether the shallow-depth sign reversal found in the original data is a reproducible phenomenon, a property specific to that dataset's calibration, or genuinely seed-dependent; this study's replication was not designed in advance to test depth-moderation specifically; a follow-up expressly using replicate-count calibration matched to $D=1$'s known convergence difficulty (Section~\ref{sec:results-calibration}) would be a targeted way to move past the disagreement observed at $D=1,2$. Beyond these two priorities, a continuous-state (non-self-contained-block) control experiment for the $E\times L$ comparison would clarify whether the super-additive interaction found here is a property of the measure--re-encode--reset architecture specifically or would carry over to conventional layered circuits with a single evolving state. A hardware-noise study is a natural further step, since every result here is from noiseless simulation. Testing additional residual-connection implementations (variable or trainable gain, alternative shortcut placements) and additional local-objective formulations would clarify whether H3's near-zero estimate and H4's fragile signal are properties of this specific fixed-gain design or would look different under other reasonable choices. Finally, extending to larger qubit counts with adequate theoretical support---rather than a numerical sweep alone---remains the only way to connect this few-qubit operational picture back to the asymptotic barren-plateau question that motivated it in the first place.
...
In the four-qubit measure--re-encode--reset case study, restricted entanglement and the TFIM-energy objective exhibit a positive exact-gradient interaction supported by both mixed-effects and matched-bootstrap analyses, and this exact-gradient result remains the study's most robust finding. The analogous finite-shot estimator-SNR interaction remains formally rejected under the prespecified Wald/Holm procedure, but that significance is not corroborated by three post-run robustness checks that relax the mixed model's assumptions in different ways, and the interaction's sign is not uniform across block count in a single joint model. An independently seeded replication provides evidence for a reproducible positive pooled direction under new initialization and shot seeds, with unresolved magnitude (roughly double the original) and unresolved depth dependence (agreeing with the original at 3 of 5 block counts). The finite-shot result is therefore not model-independent: which inference method, which pooling scheme, and which dataset one consults changes the picture in ways the pooled coefficient alone does not reveal. No robust interaction involving the fixed residual shortcut is identified, and combining all three strategies does not consistently outperform the best single intervention.

The main conclusion is methodological as well as empirical: QNN trainability interventions should be treated as potentially interacting components rather than assumed to compose additively, and factorial analyses of such interactions should budget for testing whether an interaction coefficient is itself moderated by a third design dimension---here, block count---rather than assuming one pooled coefficient applies uniformly. At the same time, exact-gradient magnitude, finite-shot resolvability, estimator fidelity, and optimization performance are distinct outcomes and should not be collapsed into a single claim of ``trainability.'' The released code, frozen data, audit records, and regression tests provide a reproducible basis for extending the protocol to more independent initializations, an explicit depth-moderation replication, continuous-state architectures, hardware-noise models, optimization trajectories, alternative objectives and residual designs, and larger qubit counts.
...
\medskip \noindent\textbf{A.6---Regression-test suite and reproducibility closure.} Beyond the test in A.5, eight further regression tests were added covering: mixed-mode confirmatory input raising by default; the $D\neq1$ sensitivity path preserving the full-sweep block-count scaling; non-overlapping bootstrap shard seed ranges; idempotent resumption of an interrupted bootstrap; correct exclusion (with reporting) of failed fits from bootstrap summaries; confirmatory forest-plot inputs using only end-to-end bootstrap draws; reproduction of the production zero-variance eligibility count; and invariance of the adopted coefficients to reporting-only code changes. The full test suite (140 tests) passed at the time of this package; the suite has since grown to 233 tests (additional coverage from the Figure~\ref{fig:el-primary} regeneration and the H2 robustness/replication/depth-heterogeneity package described below), all passing, and \ttf{pytest tests/} reproduces this count from a clean checkout. The adopted coefficients ($\beta_{EL}=0.024995843985971582$, $\beta_{ER}=-0.0009575787575784316$, $\beta_{LRd}=-0.010178757716721849$) were reproduced bit-for-bit from the frozen input files before any of the post-run sensitivity analyses in Section~\ref{sec:zero-variance-sensitivity} were run, and no quantum simulation was regenerated at any point during the audit: every check in this appendix and in Table~\ref{tab:repro-index} operates on the existing pointwise and raw replicate-level outputs.

\medskip \noindent\textbf{A.7---H2 robustness and independent replication.} A post-run package examined whether the H2 estimator-SNR interaction's statistical significance is robust to methods other than the prespecified mixed model, and whether it reproduces under an independently seeded dataset. A numerator/denominator decomposition found both the gradient-mean magnitude and the repeated-shot SD contribute to the interaction in a mutually reinforcing direction, stable under leave-one-initialization-out. A cluster-robust reanalysis (initialization-clustered OLS, identical fixed-effect formula) and a separately implemented initialization-level resampling procedure (distinct from, and cross-checking, the production bootstrap code) both produced 95\% intervals including zero, joining the existing nested bootstrap as a third line of evidence that the mixed model's rejection does not survive relaxing its variance assumptions. An independently seeded replication ($R_{\mathrm{rep}}=30$, matching the original exactly; seed root verified non-overlapping with the original and with every other config in this repository) reproduced the pooled interaction's positive sign and statistical significance at roughly twice the original's magnitude ($\beta_{EL}=0.049294$ versus $0.024996$, $3.34$ original-standard-error units apart)---classified, under a decision rule fixed before the replication was inspected, as ``direction replicated but magnitude uncertain,'' not confirmed. Full plan, results, and machine-readable outputs: \ttf{verification/h2\_robustness\_replication\_plan.md}, \ttf{verification/h2\_robustness\_replication\_results.md}, \ttf{results/h2\_robustness/}, \ttf{results/h2\_replication\_v1/}.

\medskip \noindent\textbf{A.8---H2 depth-heterogeneity analysis.} A further post-run package tested whether the pooled $E{:}L$ interaction conceals block-count-dependent effect modification, using a single categorical mixed model with an added $E{:}L{:}C(D)$ moderation term (Section~\ref{sec:h2-depth-methods}) rather than the five independent per-depth fits used in A.7's companion diagnostic. A joint Wald test on the four moderation coefficients, programmatically identified from the fitted model's own design information, rejected the null of no depth-varying interaction in both the original data ($\chi^2=154.05$, $\mathrm{df}=4$, $p=2.8\times10^{-32}$) and the independent replication ($\chi^2=37.77$, $p=1.3\times10^{-7}$), each individually significant, though this joint test does not survive a cluster-robust reanalysis in either dataset (original $p=0.15$; replication $p=0.59$). Every one of the five depth-specific contrasts in the original data individually excludes zero, with a sign reversal at $D=1,2$ not present in the replication (positive at every depth there); the two datasets' depth-specific patterns agree at 3 of 5 block counts. Equal-depth and observation-count-weighted pooled contrasts disagree materially in the original data. Every zero-variance cell's repeated-shot mean was verified to be exactly zero before an include-as-zero sensitivity treatment was applied; that treatment left every sign and significance determination unchanged. Full plan, results, code, figure, and table: \ttf{verification/h2\_depth\_heterogeneity\_plan.md}, \ttf{verification/h2\_depth\_heterogeneity\_results.md}, \ttf{qnn\_snr/stats/depth\_heterogeneity.py}, Figure~\ref{fig:h2-depth-heterogeneity}, Table~\ref{tab:h2-depth-heterogeneity}.
...
App.~\ref{app:verification}, A.6 & Full regression-test suite (233 tests) & \ttf{pytest tests/} & \ttf{qmi\_qip\_robustness\_package.md} \\
App.~\ref{app:verification}, A.7 & H2 zero-variance/decomposition audit, cluster-robust and resampling sensitivity, independent replication & \ttf{run\_h2\_zero\_variance\_audit.py}, \ttf{run\_h2\_decomposition.py}, \ttf{run\_h2\_robust\_inference\_fast.py}, \ttf{run\_h2\_init\_level\_resampling.py}, \ttf{run\_h2\_replication\_stage1.py} & \ttf{h2\_robustness\_replication\_plan.md}, \ttf{h2\_robustness\_replication\_results.md} \\
App.~\ref{app:verification}, A.8 & H2 depth-heterogeneity: continuous/categorical moderation, joint omnibus test, weighted contrasts, cluster-robust and include-zero sensitivity & \ttf{run\_h2\_depth\_heterogeneity.py} (\ttf{qnn\_snr/stats/depth\_heterogeneity.py}) & \ttf{h2\_depth\_heterogeneity\_plan.md}, \ttf{h2\_depth\_heterogeneity\_results.md} \\
\bottomrule
\end{tabularx}
\end{table*}
