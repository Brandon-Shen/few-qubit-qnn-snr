...
\AtBeginDocument{\hypersetup{
  pdftitle={Online Resource 1: Factorial Benchmarking of Exact and Finite-Shot Gradient Resolvability in a Four-Qubit Hybrid Quantum Neural Network},
  pdfauthor={Brandon Shen},
  pdfsubject={Supplementary Information for SN Computer Science}
}}

% Clean single-author correspondence formatting produced by sn-jnl.cls.
% Keep the conventional author asterisk while avoiding a duplicated asterisk
% on the affiliation, plural labels, and a trailing semicolon after the email.
\makeatletter
\patchcmd{\@maketitle}
  {Corresponding author(s). E-mail(s):}
  {Corresponding author:}
 {}{\PackageWarning{supplemental}{Could not patch corresponding-author label}}
\patchcmd{\email}
  {\textcolor{blue}{#1};\ }
  {\textcolor{blue}{#1}}
 {}{\PackageWarning{supplemental}{Could not remove corresponding-email semicolon}}
\makeatother
\begin{document}
...
\midrule
\input{../results/primary_corrected/effect_coded/primary_h1_h4_rows.tex}
\bottomrule
\end{tabularx}
\caption{Corrected primary family. Wald intervals and Holm-adjusted $p$-values follow the primary model-based decision rule; the Bootstrap 95\% CI column gives initialization-cluster percentile intervals. ``Both'' means that the Wald and bootstrap intervals exclude zero; ``Wald only'' means that the Wald/Holm rule rejects while the bootstrap interval includes zero; and ``Neither'' means that the Wald/Holm rule does not reject and the bootstrap interval includes zero. H1 used 2,000 completed fits; H2--H4 used exactly 1,000 unique valid fits. Bootstrap iterations are distinct from the 50 independent initialization clusters.}
...
The final H2--H4 pool preserves the earlier 443 successful draws unchanged (8 validated draws plus three independently seeded 145-draw shards) and appends non-overlapping deterministic continuation draws to reach exactly 1,000 unique valid fits under the revised stopping instruction. A further 178 valid fits completed before workers stopped; they are preserved in the superseded/audit location and excluded from every reported interval. The 443 result is retained as an audit checkpoint rather than described as final. Attempted, included-successful, excluded-successful, failed, and rejected counts, global iteration IDs, shard seeds, convergence status, and coefficient vectors are recorded in the machine-readable provenance files. These are completed bootstrap fits, not a sample size or a count of independent initialization clusters. Percentile endpoints retain finite Monte Carlo uncertainty.
...
\midrule
\input{../results/production_corrected_end_to_end/bootstrap_checkpoint_rows.tex}
\bottomrule
\end{tabular}
\caption{H2--H4 nested-bootstrap percentile endpoint checkpoints. The 443 row is the preserved historical checkpoint; 1,000 is the final included fit count. Rows are cumulative completed refits of resampled data, not independent datasets; endpoint movement reflects finite Monte Carlo uncertainty. Order-statistic/binomial-rank endpoint diagnostics in the machine-readable summary quantify Monte Carlo uncertainty and are not scientific confidence intervals.}
...
\bottomrule
\end{tabular}
\caption{Post-primary H3 explanatory and sensitivity results. The 1,000-fit bootstrap intervals contain zero for both objective-specific contrasts and their average. Active-depth and conditional-mode rows are not substitutions for the primary model.}
\end{table*}
...
\href{https://github.com/Brandon-Shen/few-qubit-qnn-snr/tree/sncs-submission-v3}{\texttt{sncs-submission-v3}}
...
Limitations include four qubits; one open-boundary TFIM task; reset-per-block architecture; one implementation per intervention; one initialization family; 50 top-level clusters per dataset; exact-state and abstract shot simulation; no hardware noise, readout, calibration, or optimization trajectories; prepared-state metrics only at initialization; $L$-linked finite-replicate zero-variance selection; depth weighting from parameter count; covariance-sensitive depth conclusions; H2/H3 procedure disagreement; H3 estimator-mode disagreement; weighting-dependent conditional J; absence of a locally archived full protocol source; finite Monte Carlo uncertainty in the 1,000-fit percentile endpoints; and no matched physical-resource claim.
